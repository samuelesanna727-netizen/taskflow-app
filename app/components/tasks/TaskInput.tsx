...
export default function TaskInput({
  ...
  return (
    <div className="bg-[#0A0A0A] p-4 rounded-[1rem] border border-white/5 mb-10 relative">
      ...
      {/* INPUT BOX - Il bordo diventa rosso se c'è un errore */}
      <div className={`bg-[#0D0D0D] p-2 rounded-[1rem] flex flex-col lg:flex-row gap-3 items-center border transition-all duration-300 relative z-20 ${
        ...
        {/* INPUT TESTO - Il placeholder cambia dinamicamente se c'è un errore */}
        ...
        <div className="relative w-full lg:w-auto">
          <button
            ...
            className={`flex items-center justify-between w-full lg:w-[140px] bg-[#111111] border text-[11px] px-4 py-2.5 rounded-xl font-bold transition-all outline-none ${
              error ? "border-red-500/20 text-red-400/50" : "border-white/10 text-gray-400 hover:border-[#2DD4BF]/50"
            }`}
          >
            <span>Priority: {priorityOptions.find(o => o.val === priority)?.label}</span>
            <svg className={`w-3 h-3 transition-transform duration-200 ${isPriorityOpen ? 'rotate-180 text-[#2DD4BF]' : ''}`} fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
          ...
          {isPriorityOpen && (
            <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[#111111] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-30 animate-in fade-in zoom-in-95 duration-150">
              {priorityOptions.map((opt) => (
                <button
                  key={opt.val}
                  onClick={() => { setPriority(opt.val as Priority); setIsPriorityOpen(false); }}
        ...
        <div className="relative w-full lg:w-auto">
          ...
          >
            <span>{category}</span>
            <svg className={`w-3 h-3 transition-transform duration-200 ${isCategoryOpen ? 'rotate-180 text-[#2DD4BF]' : ''}`} fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
        ...
        <button
          ...
          className={`w-full lg:w-auto flex items-center justify-center px-6 py-2.5 rounded-xl font-black text-sm transition-all shadow-lg ${
            error 
              ? "bg-red-500 text-white shadow-red-500/20 opacity-50 cursor-not-allowed" 
